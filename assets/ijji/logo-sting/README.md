...
The component plays once when visible, then holds the complete final state. It also exposes `play()`, `pause()`, `seek(seconds)`, `finish()`, and `replay()` for controlled previews. Do not add `loop` to production solely because the library Preview loops.
